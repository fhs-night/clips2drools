...
class droolsemitter(clipsparserListener):
    ...
    def setrule(self, lhs, rhs, rule_name):
        ...
        #     lhs += 'String(this in ("%s.drl")) from $CurrentName\n' % self.filename
        # lhs += 'String(this in ("%s.drl")) from $LastName\n' % self.last_filename
        ...
        buf += 'rule "%s"\n' % rule_name
        # 添加规则属性
        # if (self.filename == 'Entrance' or self.filename == 'MS_DietDataJudge'
        #         or self.filename == 'MS_SportDataJudge'
        #         or self.filename == 'MS_MSRiskDegreeEvaluation'):
            ...
            if self.formatText_number > 0:
                if ('MS_DM' in self.filename) or (self.ms_ms_mergexml == '糖尿病'):
                    ...
                    buf += '\t\t$MDTModel.getProcess("北肿诊断").getScenario("肿瘤多学科门诊决策").getProblem("入院筛查").addTask(' \
                           '$Task);\n '
                    buf += '\t}\n'
                    buf += '\tif ($MDTModel.getProcess("北肿诊断").getScenario("肿瘤多学科门诊决策").getProblem("入院筛查").getTaskById("糖尿病").getExplanation()==null){\n'
                    ...
                    buf += '\t\t$MDTModel.getProcess("北肿诊断").getScenario("肿瘤多学科门诊决策").getProblem("入院筛查").addTask(' \
                           '$Task);\n '
                    buf += '\t}\n'
                    buf += '\tif ($MDTModel.getProcess("北肿诊断").getScenario("肿瘤多学科门诊决策").getProblem("入院筛查").getTaskById("高血压").getExplanation()==null){\n'
                    buf += '\t\tFormatText $formatText_explanation = new FormatText();\n'
                    buf += '\t\t$formatText_explanation.setValue("血压 " + $Patient.getLabTestResult(' \
                           '"DBP_Top_Variable").getResult() + "-" + $Patient.getLabTestResult(' \
                           '"SBP_Top_Variable").getResult() + " " + $Patient.getLabTestResult("SBP_Top_Variable").getUnit());\n '
                    buf += '\t\t$formatText_explanation.setType(8);\n'
                    buf += '\t\tDescription explanation = new Description();\n'
                    buf += '\t\texplanation.addFormatText($formatText_explanation);\n'
                    buf += '\t\tArrayList<Description> explanationList = new ArrayList<>();\n'
                    ...
                    buf += '\t\t$MDTModel.getProcess("北肿诊断").getScenario("肿瘤多学科门诊决策").getProblem("入院筛查").addTask(' \
                           '$Task);\n '
                    buf += '\t}\n'
                    buf += '\tif ($MDTModel.getProcess("北肿诊断").getScenario("肿瘤多学科门诊决策").getProblem("入院筛查").getTaskById("血脂紊乱").getExplanation()==null){\n'
    ...
    def exitDefrule(self, ctx: clipsparser.DefruleContext):
        ...
        if ('MS_DM' in self.filename) or (self.ms_ms_mergexml == '糖尿病'):
            ...
            rhs += '\t$MDTModel.getProcess("北肿诊断").getScenario("肿瘤多学科门诊决策").getProblem("入院筛查").addTask($datanotice_task);\n'  # 待调整
            rhs += '\t}\n'
        elif ('MS_Hypertension' in self.filename) or (self.ms_ms_mergexml == '高血压'):
        ...
        if attr_name in self.data_model:
            if self.data_model[attr_name] == 'PatientInfo' or self.data_model[attr_name] == 'VisitInfo':
                if self.patient_variable[attr_name] == 'double':
                    ...
                    self.convert_string += '}\n'
                elif self.patient_variable[attr_name] == 'boolean':
                    self.convert_string += 'boolean %s = Boolean.parseboolean( $%s.get%s() );\n' % (
                        attr_value, self.data_model[attr_name], captital_name)
                ...
                if self.patient_variable[attr_name] == 'double':
                    ...
                    self.convert_string += 'boolean %s = Boolean.parseboolean( $%s_%s.getValue() );\n' % (
                        attr_value, self.data_model[attr_name], attr_name)
            ...
            if attr_name in self.patient_variable:
                ...
                if attr_name in self.data_model:
                    ...
                    # 待调整
                        ...
                        x += '$pasthistory_diagnosis_%s : ArrayList() from collect (Diagnosis(itemName == "%s") from ' \
                             '$pasthistory_diagnosis)' % (attr_name, attr_name)
